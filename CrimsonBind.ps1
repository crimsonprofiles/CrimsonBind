...
<#
.SYNOPSIS
  CSV-driven keybind manager: export/import config.ini, filter by section, randomize keys,
  update config.ini and CrimsonAutoBindDB.lua (no WoW required).

.DESCRIPTION
  Use a single CSV (Section, ActionName, Key, MacroText) as source of truth. Export from
  config.ini to CSV, edit in Excel, then Update Config and/or Update BindPad (writes
  CrimsonAutoBindDB.lua so WoW can Sync to BindPad). Filter dropdown shows General vs
  class/spec. Randomize assigns keys from a pool. Debounce: generate Lua snippet to run
  in WoW or sync in-game.

  CSV format matches bindpad_defaults.csv: Section, ActionName, MacroText, BindPadKey, TextureID
  (we use BindPadKey as Key; MacroText optional for config.ini value-after-semicolon.)

.EXAMPLE
  .\Manage-CrimsonBinds.ps1
#>
...
function Write-CrimsonBindLog {
    ...
    try {
        ...
        $logFile = Join-Path $logDir "Manage-CrimsonBinds_$(Get-Date -Format 'yyyy-MM-dd').log"
...
function Export-RowsToCsv {
    ...
    [void]$sb.AppendLine("Section,ActionName,MacroText,BindPadKey,TextureID")
    foreach ($r in $Rows) {
...
# ---------- Write CrimsonAutoBindDB.lua from rows ----------
...
function Write-CrimsonAutoBindDBFromRows {
    ...
    $sb = [System.Text.StringBuilder]::new()
    [void]$sb.AppendLine("-- Generated by Manage-CrimsonBinds.ps1. Load in WoW then Sync to BindPad.")
...
# ---------- Backup / Restore ----------
$BackupSavedVarNames = @("Debounce.lua", "Debounce.lua.bak", "BindPad.lua", "BindPad.lua.bak")
...
# ---------- Debounce.lua (BindPad/Debounce addon) ----------
...
$btnRestore.Add_Click({
    ...
    try {
        ...
        if ($Script:WtfDir -and $account) {
            $lblStatus.Text = "Restored $n file(s). config.ini and SavedVariables (Debounce/BindPad) overwritten."
        } else {
